...
const createBanner = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json(
        new errorResponse(500, `Error, create banner Controller`, error, null)
      );
  }
};
//get all banner
  ...
  try {
    ...
    return res
      ...
      .json(
        new succssResponse(
          200,
          "All Banner retrive successfull",
          false,
          searchAllBanner
        )
      );
  ...
  try {
    ...
    if (!searchSingleBanner) {
      return res
        .status(401)
        .json(
          new errorResponse(401, `search single banner failed`, true, null)
        );
    }
    return res
      .status(200)
      .json(
        new succssResponse(
          200,
          "Single Banner retrive successfull",
          false,
          searchSingleBanner
        )
      );
    ...
    return res
      .status(500)
      .json(
        new errorResponse(500, `Error,From get single banner`, error, null)
      );
  }
};
//update banner
const updateBanner = async (req, res) => {
  try {
    ...
    if (!findMyDatabase) {
      return res
        ...
        .json(
          new errorResponse(
            401,
            `Couldn't find this id from database`,
            true,
            null
          )
        );
    ...
    const saveDatabase = await bannerModel.findByIdAndUpdate(
      { _id: id },
      { ...updateNameImg },
      { new: true }
    );
    if (!saveDatabase) {
      return res
        .status(500)
        .json(
          new errorResponse(500, `Couldn't save this on database`, true, null)
        );
    }
    return res
      .status(200)
  ...
  try {
    ...
    if (!findBanner) {
      return res
        .status(401)
        .json(
          new errorResponse(401, `Couldn't delete this banner`, true, null)
        );
    }
    if (findBanner) {
      ...
      if (!delImgFromCloud) {
        return res
          .status(401)
          .json(
            new errorResponse(401, `img is deleted from cloudinary`, true, null)
          );
      }
      if (delImgFromCloud) {
    ...
    return res
      .status(200)
      .json(
        new succssResponse(200, "banner Deleted successfully", false, null)
      );
  } catch (error) {
    return res
      .status(500)
