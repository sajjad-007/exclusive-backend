  ...
  try {
    ...
    if (!saveSubCategory) {
      return res
        .status(401)
        .json(
          new errorResponse(401, `sub category create failed!`, true, null)
        );
    }
    return res
      .status(200)
    ...
    return res
      .status(500)
      .json(
        new errorResponse(500, `sub category create Unsuccessfull`, error, null)
      );
  }
};
...
const getAllSubCategory = async (req, res) => {
  try {
    ...
    const findAllSubCategory = await subCategoryModel
      .find({})
      .populate("category")
      .populate("product");
    ...
    return res
      .status(500)
      .json(
        new errorResponse(500, `get all sub category Unsuccessfull`, true, null)
      );
  }
};
...
const getSingleSubCategory = async (req, res) => {
  try {
    const { subid } = req.params;
    const findSingleSubCategory = await subCategoryModel
      .findById(subid)
      .populate("category")
      .populate("product");
    if (!findSingleSubCategory) {
      return res
        ...
        .json(
          new errorResponse(
            401,
            `Couldn't find anything by this id`,
            true,
            null
          )
        );
    ...
    return res
      ...
      .json(
        new succssResponse(
          200,
          "Successfully Get Single Sub Category ",
          false,
    ...
    return res
      .status(500)
      .json(
        new errorResponse(500, `Get single sub category failed`, true, null)
      );
  }
};
...
const updateSingleCategory = async (req, res) => {
  try {
    ...
    const findCategory = await categoryModal.findById(
